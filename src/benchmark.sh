    ...
    for temperature in "${temperatures[@]}"; do
      for model in "${models[@]}"; do
        for context_length in "${context_lengths[@]}"; do
          ...
          python src/agents/eval_agent.py --env_id "$env" --model_name "$model" --context_length "$context_length" --prompt_chain "$prompt_chain" --temperature "$temperature" --num_episodes=5
        done
      done
    done
  done
